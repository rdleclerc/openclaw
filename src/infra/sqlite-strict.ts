...
function readMainTableList(db: DatabaseSync): TableListRow[] {
  return (
    db /* sqlite-allow-raw: canonical STRICT schema migration boundary */
...
function readTableColumns(db: DatabaseSync, tableName: string): TableColumnRow[] {
  return db /* sqlite-allow-raw: canonical STRICT schema migration boundary */
...
function readTableRowidModel(
  ...
  const primaryKeyIndex = db /* sqlite-allow-raw: canonical STRICT schema migration boundary */
...
function readAutoincrementHighWater(db: DatabaseSync, tableName: string): string | null {
  const sequenceTable = db /* sqlite-allow-raw: canonical STRICT schema migration boundary */
    .prepare(
  ...
  const row = db /* sqlite-allow-raw: canonical STRICT schema migration boundary */
...
function restoreAutoincrementHighWater(
  ...
  db /* sqlite-allow-raw: canonical STRICT schema migration boundary */
    ...
    .run(tableName);
  db /* sqlite-allow-raw: canonical STRICT schema migration boundary */
    .prepare("INSERT INTO sqlite_sequence (name, seq) VALUES (?, CAST(? AS INTEGER))")
    .run(tableName, restored);
}
...
export function migrateSqliteSchemaToStrictInTransaction(
  ...
  db.exec(schemaSql); // sqlite-allow-raw: canonical STRICT schema migration boundary
  ...
  for (const object of preservedObjects) {
    if (object.type === "trigger") {
      db /* sqlite-allow-raw: canonical STRICT schema migration boundary */
        .exec(`DROP TRIGGER ${quoteSqliteIdentifier(object.name)};`);
    }
  }
  for (const object of preservedObjects) {
    if (object.type === "view") {
      db /* sqlite-allow-raw: canonical STRICT schema migration boundary */
        .exec(`DROP VIEW ${quoteSqliteIdentifier(object.name)};`);
    }
  }
  for (const [index, table] of tablesToMigrate.entries()) {
    ...
    const previousHighWater = table.usesAutoincrement
      ...
      : null;
    db /* sqlite-allow-raw: canonical STRICT schema migration boundary */
      .exec(rewriteCreateTableName(table.createSql, migrationTable));
    ...
    db /* sqlite-allow-raw: canonical STRICT schema migration boundary */
      .exec(`DROP TABLE ${quoteSqliteIdentifier(table.name)};`);
    db /* sqlite-allow-raw: canonical STRICT schema migration boundary */
      .exec(
  ...
  db.exec(schemaSql); // sqlite-allow-raw: canonical STRICT schema migration boundary
  const findObject = db /* sqlite-allow-raw: canonical STRICT schema migration boundary */
    .prepare("SELECT 1 AS found FROM sqlite_schema WHERE type = ? AND name = ? LIMIT 1");
  for (const object of preservedObjects) {
    if (!findObject.get(object.type, object.name)) {
      db.exec(object.sql); // sqlite-allow-raw: canonical STRICT schema migration boundary
    }
  }
...
export function migrateSqliteSchemaToStrict(
  ...
  if (foreignKeysWereEnabled) {
    db.exec("PRAGMA foreign_keys = OFF;"); // sqlite-allow-raw: canonical STRICT schema migration boundary
  }
  try {
    ...
    if (foreignKeysWereEnabled) {
      db.exec("PRAGMA foreign_keys = ON;"); // sqlite-allow-raw: canonical STRICT schema migration boundary
    }
  }
}
