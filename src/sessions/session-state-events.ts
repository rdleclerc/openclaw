...
/** List durable ambient-group targets owned by one watcher. Failures grant no access. */
  ...
  try {
    ...
    if (params.dmScope !== "main") {
      ...
      runOpenClawStateWriteTransaction(({ db }) => {
        ...
        // can promote this pair after the read-only preflight.
    ...
    // This runs on each human group turn. Keep the steady-state path read-only.
    ...
    runOpenClawStateWriteTransaction(({ db }) => {
      ...
      if (existing) {
        // An explicit watch owns this pair. Do not downgrade it.
        registered = true;
        return;
      }
      const head = executeSqliteQueryTakeFirstSync(
