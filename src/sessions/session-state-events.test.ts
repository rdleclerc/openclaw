...
describe("session state events", () => {
  ...
  it("grants ambient group tree reads but keeps sends and explicit watches restricted", () => {
    const database = createDatabaseOptions();
    registerMainSessionGroupWatch(
      { sessionKey: group, agentId: "main", dmScope: "main" },
      database,
    );
    const a2aPolicy = createAgentToAgentPolicy({} as OpenClawConfig);
    const history = createSessionVisibilityRowChecker({
      action: "history",
      requesterSessionKey: watcher,
      visibility: "tree",
      a2aPolicy,
    });
    const send = createSessionVisibilityRowChecker({
      action: "send",
      requesterSessionKey: watcher,
      visibility: "tree",
      a2aPolicy,
    ...
    expect(history.check({ key: group })).toEqual({ allowed: true });
    expect(send.check({ key: group })).toMatchObject({ allowed: false, status: "forbidden" });

    registerSessionStateWatch({ watcherSessionKey: watcher, targetSessionKey: group }, database);
    const afterPromotion = createSessionVisibilityRowChecker({
      action: "history",
      requesterSessionKey: watcher,
      visibility: "tree",
      a2aPolicy,
    });
    expect(afterPromotion.check({ key: group })).toMatchObject({
      allowed: false,
      status: "forbidden",
    });
  });
