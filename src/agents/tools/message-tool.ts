...
const slackTarget = (value: unknown) =>
  normalizeOptionalString(value)
    ?.replace(/^(channel|group):/iu, "")
    .toUpperCase();

export function enforceSameChannelThreadMessageAction(params: {
  action: ChannelMessageActionName;
  args: Record<string, unknown>;
  toolContext?: ChannelThreadingToolContext;
  fallbackAccountId?: string;
}): void {
  const route = params.toolContext;
  if (!route?.sameChannelThreadRequired) {
    return;
  }
  const bound = slackTarget(route.currentMessagingTarget ?? route.currentChannelId);
  const requested = slackTarget(params.args.target ?? params.args.to ?? params.args.channelId);
  const thread = normalizeOptionalString(route.currentThreadTs);
  if (
    (params.action !== "read" && params.action !== "send") ||
    normalizeMessageChannel(readStringParam(params.args, "channel")) !== "slack" ||
    normalizeMessageChannel(route.currentChannelProvider) !== "slack" ||
    !bound ||
    requested !== bound ||
    !thread ||
    normalizeOptionalString(params.args.threadId ?? params.args.threadTs) !== thread ||
    normalizeAccountId(readStringParam(params.args, "accountId") ?? params.fallbackAccountId) !==
      normalizeAccountId(params.fallbackAccountId)
  ) {
    throw new Error("Message action is outside the bound Slack channel thread.");
  }
}
...
export function createMessageTool(options?: MessageToolOptions): AnyAgentTool {
  ...
  const boundSlackThreadRequired = Boolean(
    options?.messageActionTurnCapability &&
    options.agentSessionKey?.includes(":cron:") &&
    currentThreadTs &&
    normalizeMessageChannel(effectiveCurrentChannel.currentChannelProvider) === "slack",
  );
  ...
  return {
    ...
    execute: async (toolCallId, args, signal) => {
      ...
      if (boundSlackThreadRequired && trustedTurnContext?.allowedActions === undefined) {
        if (!trustedTurnContext?.toolContext?.sameChannelThreadRequired) {
          throw new Error("Bound Slack thread authority is unavailable.");
        }
        enforceSameChannelThreadMessageAction({
          action,
          args: params,
          toolContext: trustedTurnContext.toolContext,
          fallbackAccountId: trustedTurnContext.requesterAccountId ?? agentAccountId,
        });
      }
      if (
      ...
      if (toolResult && accountId) {
        toolResult.details = { ...Object(toolResult.details), effectiveAccountId: accountId };
      }
      if (
        action === "poll-vote" &&
        pollVoteEchoRoute &&
